...
import Color
import Data.Supabase exposing (Supabase, encodeSupabase, supabase, supabaseToString)
...
signUpForm =
    let
        signup : String -> String -> DataSource Supabase
    ...
    Request.oneOf
        [ -- POST JSON Body
          Decode.map2 signup
            ...
            |> (Decode.map << DataSource.map)
                (\supaResp ->
                    supaResp
                        |> encodeSupabase
                        |> Response.json
                )
            |> Request.expectJsonBody
        , -- POST FORM Body
          Request.expectFormPost
            (\{ field, optionalField } ->
                Request.map2 signup
                    (field "email")
                    (field "password")
                    |> (DataSource.map >> Request.map) (encodeSupabase >> Response.json)
            )
        ]
        -- Build Path
        |> ApiRoute.succeed
